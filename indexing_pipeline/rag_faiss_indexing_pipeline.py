...
def build_vector_store(chunk_documents, embeddings: OpenAIEmbeddings) -> FAISS:
    if faiss is None:
        raise RuntimeError(
...
def run_indexing_pipeline(config: IndexConfig, *, rebuild: bool = True) -> None:
    ...
    if rebuild:
        ...
        # 3) Knowledge base
        vector_store = build_vector_store(chunk_documents, embeddings)
        print(f"\n[KNOWLEDGE BASE] Indexed vectors (index.ntotal): {vector_store.index.ntotal}")

        # 4) STORE
        save_vector_store(vector_store, config.output_dir)
...
def print_info(config: IndexConfig, rebuild: bool):
